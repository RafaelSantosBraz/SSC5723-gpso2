...
As instruções devem ser inseridas antes de executar o sistema, e ficam armazenadas no arquivo [config.txt](https://github.com/RafaelSantosBraz/SSC5723-gpso2/blob/master/Modulo03/simulador_memoria/config.txt).
...
* **TAG da instrução:** Define a instrução a ser executada. Deve ser uma das descritas acima, representada pela TAG **C**, **R**, **W**, **P** ou **I**.
* **tamanho do processo / endereço de memória / ID do dispositivo de E/S:** Quando a TAG **C** é usada, este item indica o tamanho (em Kbytes) da imagem do processo. Se a TAG for **R**, **W** ou **P**, este item indica o valor decimal de um endereço (em binário) lógico de memória na tabela de páginas do processo. Com a TAG **I**, este item indica o valor decimal do identificador (em binário) de um dispositivo de E/S. 
...
* **P1 W (4)2** - Escreve na memória na posição indicada pelo endereço lógico 4 (em binário)
* **P1 P (4096)2** - Acessa e executa instrução armazenada na memória indicada pelo endereço 4096 (em binário)
...
Algumas configurações do simulador podem ser definidas manualmente antes da execução por meio de modificações no arquivo [config.h](https://github.com/RafaelSantosBraz/SSC5723-gpso2/blob/master/Modulo03/simulador_memoria/config.h).

O tamanho em Kbytes da **página virtual** e do **quadro de página** da memória devem ser iguais, e são definidos na instrução:
  ```
 ...
 * Tamanho da memória virtual: 2000 páginas;
 ...
 A sequência de instruçõe a seguir é um exemplo que aborda quase todas as políticas adotadas. Sua saída foi copiada para o arquivo [ex1.txt](https://github.com/RafaelSantosBraz/SSC5723-gpso2/blob/master/Modulo03/examples/ex1.txt). Neste exemplo, o algoritmo LRU foi usado para tratar a substituição de página.
